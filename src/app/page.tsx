...
const CaroselG = dynamic(() => import("@/components/carosel-g"), {
  ssr: false,
});
const InfoCard = dynamic(() => import("@/components/info-card"), {
  ssr: false,
});
const LastAnnounce = dynamic(() => import("@/components/last-announce"), {
  ssr: false,
});
const MainCard = dynamic(() => import("@/components/main-card"), {
  ssr: false,
});
  ...
  try {
    const response = await fetch(
      `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
        address
      )}&key=AIzaSyC9gd59nW47Bg63ksUnNd2HmigKDUDGA7E`
    );
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const sortedProfiles = combinedProfiles.sort(
          (a: Profile, b: Profile) => {
  ...
  return (
    <div className="text-gray-600 bg-black w-full overflow-x-hidden ">
      ...
      <p className="text-pink-800 text-2xl flex justify-center pb-5 w-full h-[50px]">
        {" "}
        {/* Definindo altura fixa para o texto */}
        Procura na tua Área
      </p>
